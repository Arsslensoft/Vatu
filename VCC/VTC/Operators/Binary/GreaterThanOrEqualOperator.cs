...
namespace VTC
{
    ...
    public class GreaterThanOrEqualOperator : BinaryOp
    {
        ...
        public override bool EmitBranchable(EmitContext ec, Label truecase, bool v)
        {
            ...
            if (Left.Type.Size == 1)
                ec.EmitInstruction(new Compare() { DestinationReg = ec.GetLow(LeftRegister.Value), SourceReg = ec.GetLow(RightRegister.Value), Size = 8, OptimizingBehaviour = OptimizationKind.PPO });
            else
                ec.EmitInstruction(new Compare() { DestinationReg = LeftRegister.Value, SourceReg = RightRegister.Value, Size = 16, OptimizingBehaviour = OptimizationKind.PPO });
