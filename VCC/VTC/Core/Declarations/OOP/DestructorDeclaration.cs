...
namespace VTC.Core
{
    public class DestructorDeclaration : Declaration
    {
        ...
        [Rule(@"<Destructor Decl> ::= <Mod> ~'~'Id <Func Body>")]
        public DestructorDeclaration(Modifier m, Identifier id, FunctionBodyDefinition fbd)
        {
            _name = id;
            mod = m;
            _fbd = fbd;
 ...
 public override SimpleToken DoResolve(ResolveContext rc)
        {
          ...
          //  base._type.Type = BuiltinTypeSpec.Void;
            if (rc.CurrentType.Name != _name.Name)
                ResolveContext.Report.Error(0, Location, "Destructor's name must match the current type name");

          
                _fbd.ParamTypes.Insert(0, rc.CurrentType);

            method = new MethodSpec(rc.CurrentNamespace, _name.Name+"_$dtor", mods, BuiltinTypeSpec.Void, ccv, _fbd.ParamTypes.ToArray(), this.Location);
    ...
    int paramidx = (p.MemberType.Size == 1) ? 2 : p.MemberType.Size;
     ...
     return paramidx + p.StackIdx;
        ...
        public override bool Emit(EmitContext ec)
        {
            ...
            mlb.Method = true;
            ec.MarkLabel(mlb);
            ec.EmitComment("Destructor: Name = " + method.Name );
            // create stack frame
