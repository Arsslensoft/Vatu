...
namespace VTC.Core
{
    public class PropertyDeclaration : Declaration
    {
     ...
     if(_setter != null){
         ...
         // Calling Convention
         ccvh.SetParametersIndex(ref SetterParameters, ccv);
...
//         rc.CurrentMethod = property.Setter;
     ...
     if (_getter != null)
     {
         ...
         // Calling Convention
         ccvh.SetParametersIndex(ref GetterParameters, ccv);
         if (ccv == CallingConventions.FastCall)
     ...
     if (_getter != null && _setter != null && !(_setter.AutoGenerated && _setter.AutoGenerated))
