...
namespace VTC
{
   public class SByteEmitter : ReferenceSpec
    {
    
       public SByteEmitter(MemberSpec ms,int off, ReferenceKind k) : base( k)
       {
      
           Member = ms;
           Offset = off;
       }
       public SByteEmitter(MemberSpec ms, int off, ReferenceKind k, RegistersEnum reg)
           : this(ms,off,k)
       {
           Register = reg;
       ...
       public override bool EmitToStack(EmitContext ec)
       {
           ...
           if (ReferenceType == ReferenceKind.Field)
           {
               ...
               ec.EmitInstruction(new MoveSignExtend() { DestinationReg = EmitContext.D, SourceRef = ElementReference.New(Signature.ToString()), SourceDisplacement = Offset, SourceIsIndirect = true, Size = 8 });
           ...
           else if (ReferenceType == ReferenceKind.LocalVariable)
           {
               ...
               ec.EmitInstruction(new MoveSignExtend() { DestinationReg = EmitContext.D, SourceReg = EmitContext.BP, SourceDisplacement = Offset, SourceIsIndirect = true, Size = 8 });
               ec.EmitInstruction(new Push() { DestinationReg = EmitContext.D, Size = 16 });
           }
           else if (ReferenceType == ReferenceKind.Register)
           {
               ec.EmitComment("Push Var @" + Register.ToString() + Offset);
               ec.EmitInstruction(new MoveSignExtend() { DestinationReg = EmitContext.D, SourceReg = Register, SourceDisplacement = Offset, SourceIsIndirect = true, Size = 8 });
               ec.EmitInstruction(new Push() { DestinationReg = EmitContext.D, Size = 16 });
           }
           else
           {
               ec.EmitComment("Push Parameter @BP " + Offset);
               ec.EmitInstruction(new MoveSignExtend() { DestinationReg = EmitContext.D, SourceReg = EmitContext.BP, Size = 8, SourceDisplacement = Offset, SourceIsIndirect = true });
               ec.EmitInstruction(new Push() { DestinationReg = EmitContext.D, Size = 16 });
           }
           return true;
       }
       ...
       public override bool ValueOf(EmitContext ec)
       {
           if (ReferenceType == ReferenceKind.Field)
           {
               ...
               ec.EmitPushSigned(EmitContext.SI, MemberType.BaseType.SizeInBits, true);
           }
           else if (ReferenceType == ReferenceKind.LocalVariable)
           {
           ...
           else if (ReferenceType == ReferenceKind.Register)
           {
               ...
               ec.EmitPushSigned(EmitContext.D, MemberType.BaseType.SizeInBits, true);
           }
           else
           {
               ec.EmitComment("ValueOf @BP+" + Offset);
       ...
       public override bool ValueOfAccess(EmitContext ec, int off, TypeSpec mem)
       {
           if (ReferenceType == ReferenceKind.Field)
           {
               ...
               ec.EmitInstruction(new Mov() { DestinationReg = EmitContext.SI, SourceIsIndirect = !memberType.IsArray, Size = 16, SourceRef = ElementReference.New(Signature.ToString()) });
               ec.EmitPushSigned(EmitContext.SI, mem.SizeInBits, true, off);
           ...
           else if (ReferenceType == ReferenceKind.Register)
           {
               ...
               ec.EmitInstruction(new Mov() { DestinationReg = EmitContext.D, SourceIsIndirect = !memberType.IsArray, Size = 16, SourceReg = Register, SourceDisplacement =  Offset });
               ec.EmitPushSigned(EmitContext.D, mem.SizeInBits, true, off);
           }
           else
           {
               ec.EmitComment("ValueOf Access @BP+" + Offset);
