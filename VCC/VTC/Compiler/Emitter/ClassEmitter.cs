...
namespace VTC
{
    public class ClassEmitter : ReferenceSpec
    {


        public ClassEmitter(MemberSpec ms, int off, ReferenceKind k)
            : base(k)
        {
        ...
        public ClassEmitter(MemberSpec ms, int off, ReferenceKind k, RegistersEnum reg)
            : this(ms, off, k)
        {
            Register = reg;
        ...
        public override bool ValueOf(EmitContext ec)
        {
            if (ReferenceType == ReferenceKind.Field)
            {
                ...
                ec.EmitPush(EmitContext.SI, (MemberType as ClassTypeSpec).Members[0].MemberType.SizeInBits, true);
        ...
        public override bool ValueOfStack(EmitContext ec)
        {
            if (ReferenceType == ReferenceKind.Field)
            {
                ec.EmitComment("ValueOf Field ");
                ec.EmitInstruction(new Mov() { DestinationReg = EmitContext.SI, SourceIsIndirect = !memberType.IsArray, Size = 16, SourceRef = ElementReference.New(Signature.ToString()) });
                ec.EmitPop(EmitContext.SI, (MemberType as ClassTypeSpec).Members[0].MemberType.SizeInBits, true);
            }
            else if (ReferenceType == ReferenceKind.LocalVariable)
            {
